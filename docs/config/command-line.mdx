...
<ParamField path="--id" type="string" env="ID">
  Instance identifier used as the default `database_id` when running a single DBHub instance. Useful when running multiple DBHub instances (e.g., in Cursor).

  Built-in tool names remain shared (`execute_sql`, `search_objects`, `list_databases`).
  ...
  Result: call `execute_sql(database_id="prod", ...)` and `execute_sql(database_id="staging", ...)`
